...
- `src/components/PageHeader`, `src/components/PageFooter` — composed from this repo's own `<Button>` via an `actions`/`meta` slot rather than re-implementing Secondary/Primary button styling. Figma's Header variant also has a credit-balance chip, a refresh-timestamp chip, and a tertiary button that read as one-off instance overrides rather than generic header needs — not built here, flagged as a gap instead of guessed at.
- `src/components/Pagination` — "1–15 of 300" plus prev/next; takes `from`/`to`/`total` as numbers rather than Figma's hardcoded string.
- `src/components/PrimaryTabs`, `src/components/SecondaryTabs` — the two tab-row styles (bordered chip vs. pill-in-a-track). Both are generic `items` + `activeKey`/`onChange`, not a fixed tab count like the Figma instances.
...
- `src/components/ProgressIndicator` — Figma's stepper is a fixed 5-tab instance; this is a generic `steps: {key,label,icon?}[]` + `activeKey`, with per-step state (`completed`/`active`/`default`) derived from list position. Steps are `flex-1` rather than Figma's fixed 160px width so any step count fits.
...
- `src/components/CreatorCard` — Instagram/YouTube creator tile. Figma models the platform-badge pair (primary + secondary platform icon) as 4 separate hardcoded conditionals; simplified here to `platform === 'instagram' ? [IconInstagram, IconYoutube] : [IconYoutube, IconInstagram]` with the first icon getting the white rounded highlight — same visual result, one branch instead of four.
- `src/components/ProfileCard` — the large single-creator profile view. `details`/`contacts`/`content` are consumer-supplied arrays rather than Figma's fixed field list; cover photo uses a new `overlay.black-80` token (see below) for the gradient over the header photo.
- `src/components/CreatorListCard` — `variant: 'single-select' | 'multi-select'` switches between the two Figma instances (Add-to-list/Delete actions vs. a selection checkbox); `thumbnails` is a `ReactNode[]` slot capped at 6 by convention, not enforced.
...
- `src/components/DiscoveryFilterPanel` — the biggest Figma→prop generalization in this repo. Figma bakes in ~18 fixed filter sections with real tenant content (brand names, city lists, price bands); every one of them is actually one of four repeating shapes, so this takes a `sections: FilterSection[]` list (`toggles` / `checkboxes` / `range` / `select-range`) and the consumer supplies which sections exist and what's in them. Reuses `Checkbox`, `ToggleSwitch`, `InputField`, `Button` rather than reimplementing any of them. The `checkboxes` shape has its own "Show More" expand state built in.
...
`navIcons.tsx`/`formIcons.tsx` actually reference: all but three
(`IconSignOut`, `IconGift`, `IconCaretLeft`/`IconCaretRight` — see above for
why) are real Figma-extracted assets, not hand-drawn stand-ins. What's
